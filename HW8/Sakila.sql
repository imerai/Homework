...
update actor 
set first_name = case when first_name = 'GROUCHO' and last_name = 'Williams' then 'MUCHO GROUCHO'
when first_name = 'HARPO' and last_name = 'Williams' then 'GROUCHO' else first_name end where last_name = 'Williams';
select * from actor where last_name = 'Williams';
...
SELECT s.staff_id, s.first_name, s.last_name, SUM(p.amount) as TotAmt
FROM staff as s
...
SELECT f.film_id, f.title, COUNT(i.inventory_id) as CntInventory
FROM film as f
...
SELECT c.customer_id, c.last_name, c.first_name, SUM(p.amount) as TotalPayments
FROM customer as c 
INNER JOIN payment as p
...
WHERE fa.film_id IN (SELECT f.film_id FROM film as f WHERE f.title = 'Alone Trip');
# 7c. You want to run an email marketing campaign in Canada, for which you will need the names and email addresses of all Canadian customers. 
# Use joins to retrieve this information.
SELECT cs.last_name, cs.first_name, cs.email
FROM customer as cs
INNER JOIN address as a
...
SELECT f.film_id, f.title, COUNT(r.inventory_id) as NumRents
FROM rental as r
INNER JOIN inventory as i
...
SELECT s.store_id, a.address , SUM(p.amount) FROM store as s
...
INNER JOIN address as a 
ON s.address_id = a.address_id
GROUP BY s.store_id, a.address ORDER BY s.store_id;
...
SELECT c.name, SUM(p.amount) as GrossRevenue FROM payment as p
...
CREATE VIEW ViewTopFiveGenres as 
...
SELECT * FROM ViewTopFiveGenres;
# 8c. You find that you no longer need the view top_five_genres. Write a query to delete it.
DROP VIEW ViewTopFiveGenres;